package fr.ign.cogit.HMMSpatialNetworkMatcher.impl.tp;
...
public class AngularTransitionProbability implements ITransitionProbabilityStrategy{
  ...
  public double compute(IObservation obs1, IHiddenState currentState,
    ...
    if(!(obs1 instanceof Observation) || !(obs2 instanceof Observation) || 
        !(currentState instanceof HiddenState) || !(nextState instanceof HiddenState)) {
      throw new RuntimeException("Parameters must extends Observation and HiddenState to"
          + "compute AngularTransitionProbability");
    }

    ILineString geomRef1 = (ILineString)((HiddenState)currentState).getGeom().clone();
    ILineString geomRef2 = (ILineString)((HiddenState)nextState).getGeom().clone();
    ILineString geomComp1 = (ILineString)((Observation)obs1).getGeom().clone();
    ILineString geomComp2 = (ILineString)((Observation)obs2).getGeom().clone();
    ...
    if(currentState.equals(nextState)){
      ...
      if(currentState instanceof CompositeHiddenState) {
        for(HiddenState a1: ((CompositeHiddenState) currentState).getStates()){
      ...
      if(nextState instanceof CompositeHiddenState) {
        for(HiddenState a1: ((CompositeHiddenState) nextState).getStates()){
          if(currentState instanceof CompositeHiddenState) {
