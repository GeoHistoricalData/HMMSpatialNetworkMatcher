...
public class IDWInterpolator {
    ...
    private static final Logger logger = Logger.getLogger(IDWInterpolator.class);
    ...
    public static void main(String[] args) {
        ...
        final BufferedImage image = IDWInterpolator.getBufferedImageInterpolation(
    ...
    public static PlanarImage getRasterInterpolation(Collection<Point2D> points,
        ...
        for (int h = 0; h < HEIGHT; h++)
...

            {
                ...
                imageData[count++] = (float) IDWInterpolator.interpole_point(pt,
    ...
    public static double interpole_point(Point2D pt,
        ...
        double somme = 0;
        double result = 0;
        for (IFeature f : mappingValuesFeatures.keySet()) {
            if(f.getGeom() instanceof IPoint){
                ...
                double distance = Math.sqrt((pt.getX() - ptf.getX())
                        ...
                        * (pt.getY() - ptf.getY()));
                if (distance < epsilon) {
                    return mappingValuesFeatures.get(f);
                } else {
                    double weight = mappingValuesFeatures.get(f);
                    distance = 1. / (Math.pow(distance, ALPHA));
                    somme += distance;
    ...
    public static double interpole_point(Point2D pt,
        ...
        double somme = 0;
        double result = 0;
        for (Point2D ptf : mappingValuesFeatures.keySet()) {
            double distance = Math.sqrt((pt.getX() - ptf.getX())
                    * (pt.getX() - ptf.getX()) + (pt.getY() - ptf.getY())
                    * (pt.getY() - ptf.getY()));
            if (distance < epsilon) {
                return mappingValuesFeatures.get(ptf);
            } else {
