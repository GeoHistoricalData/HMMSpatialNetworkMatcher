package fr.ign.cogit.morphogenesis.criteria;
...
public class Orientation {
  ...
  public static double mainOrientation(ILineString line) {
    ...
    for (int i = 1; i < coords.size(); i++) {
      ...
      // System.out.println("Tronçon orienté ) "+(bearing(p1, p2)*180/Math.PI));
  ...
  public static void main(String args[]) {
    ...
    IPopulation<IFeature> pop = ShapefileReader
        .read("/home/bcostes/Bureau/test.shp");

    IFeatureCollection<IFeature> out = new Population<IFeature>();

    for (int i = 0; i < pop.size(); i++) {

      ILineString l = new GM_LineString(pop.get(i).getGeom().coord());

      System.out
          .println("[ "
              + 180
              / Math.PI
              * Orientation.FLOrientation(l)
              + " "
              + 180
              / Math.PI
              * Orientation.meanOrientation(l)
              + " "
              + 180
              / Math.PI
              * Orientation.mainOrientation(l)
              + " "
              + 180
              / Math.PI
              * Orientation.globalOrientation(l)
              + " ]"
              + " , [ "
              + 180
              / Math.PI
              * Orientation.FLOrientation(GaussianFilter.gaussianFilter(l, 100,
                  10))
              + " "
              + 180
              / Math.PI
              * Orientation.meanOrientation(GaussianFilter.gaussianFilter(l,
                  100, 10))
              + " "
              + 180
              / Math.PI
              * Orientation.mainOrientation(GaussianFilter.gaussianFilter(l,
                  100, 10))
              + " "
              + 180
              / Math.PI
              * Orientation.globalOrientation(GaussianFilter.gaussianFilter(l,
                  100, 10)) + " ]");

      out.add(new DefaultFeature(GaussianFilter.gaussianFilter(l, 100, 10)));
