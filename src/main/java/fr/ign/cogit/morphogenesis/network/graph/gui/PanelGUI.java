...
public class PanelGUI extends JPanel {
  ...
  private Map<Shape, Color> backGround; // lorsqu'on affiche des noeuds
  ...
  public void paintComponent(Graphics g) {
    ...
    if (this.backGround != null) {
      for (Shape s : backGround.keySet()) {
        g2d.setPaint(backGround.get(s));
        g2d.draw(s);
      }
    }
  ...
  public void changeColorType(int colorType) {
    ...
    switch (this.colorType) {
      ...
      case ColorType.LENGTH_QUANTIL:
        ...
        if (this.values.keySet().iterator().next() instanceof Ellipse2D) {
          colors = new Color[INTERVAL_SIZE];
          for (int i = 0; i < INTERVAL_SIZE; i++) {
            rgb = ColorScale.HSVtoRGB(4.5 - (4.5 / (INTERVAL_SIZE - 1))
                * (double) i, 0.9, 1);
            colors[i] = new Color((float) rgb[0], (float) rgb[1],
                (float) rgb[2]);
          }

          pas = (int) ((double) this.values.size() / (double) INTERVAL_SIZE);
  ...
  public void setBackGround(Map<Shape, Color> backGround) {
    this.backGround = backGround;
  }

  public Map<Shape, Color> getBackGround() {
    return backGround;
  }
}
