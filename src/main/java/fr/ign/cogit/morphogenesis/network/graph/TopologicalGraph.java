...
public class TopologicalGraph extends AbstractGraph<Edge, String> {
  ...
  public Map<Edge, Double> getCentralityEdges(String centrality) {
    ...
    if (centrality.equals(CentralitiesDual.CLOSENESS)) {
      ...
      Map<Edge, Double> e = this.getEigenVectorCentrality();
      for (Edge edge : this.getVertices()) {
    ...
    } else if (centrality.equals(CentralitiesDual.LOCALINTEGRATION_2)) {
      if (logger.isInfoEnabled()) {
        logger.info("Calcul des intégration locales (2) ...");
      }
      Map<Edge, Double> e = this.getLocalIntegration(2);
      for (Edge edge : this.getVertices()) {
        values.put(edge, e.get(edge));
      }
    } else if (centrality.equals(CentralitiesDual.LOCALINTEGRATION_3)) {
      if (logger.isInfoEnabled()) {
    ...
    } else if (centrality.equals(CentralitiesDual.LOCALINTEGRATION_4)) {
      if (logger.isInfoEnabled()) {
        logger.info("Calcul des intégration locales (4) ...");
      }
      Map<Edge, Double> e = this.getLocalIntegration(4);
      for (Edge edge : this.getVertices()) {
        values.put(edge, e.get(edge));
      }
    } else if (centrality.equals(CentralitiesDual.LOCALINTEGRATION_5)) {
      if (logger.isInfoEnabled()) {
        logger.info("Calcul des intégration locales (5) ...");
      }
      Map<Edge, Double> e = this.getLocalIntegration(5);
      for (Edge edge : this.getVertices()) {
        values.put(edge, e.get(edge));
      }
    ...
    } else if (centrality.equals(CentralitiesDual.KATZCENTRALITY_LOCAL)) {
      if (logger.isInfoEnabled()) {
        logger.info("Calcul des centralités de Katz locales ...");
      }
      Map<Edge, Double> e = this.getKatzCentrality(1.5);
      for (Edge edge : this.getVertices()) {
        values.put(edge, e.get(edge));
      }
    } else if (centrality.equals(CentralitiesDual.KATZCENTRALITY_GLOBAL)) {
      if (logger.isInfoEnabled()) {
        logger.info("Calcul des centralités de Katz globales ...");
      }
      Map<Edge, Double> e = this.getKatzCentrality(1.0001);
      for (Edge edge : this.getVertices()) {
        values.put(edge, e.get(edge));
      }
    }
    values = (new Statistics<Edge>()).centrerRéduire(values);
    return values;
  }
  ...
  @Override
  public Map<Edge, Double> getLocaleClosenessCentrality(int depth) {
    // TODO Auto-generated method stub
    return null;
  }

  @Override
  public Map<Edge, Double> getLocaleBetweennessCentrality(int depth) {
    // TODO Auto-generated method stub
    return null;
  }

  @Override
  public Map<Edge, Double> getFlowBetweennessCentrality() {
    // TODO Auto-generated method stub
    return null;
  }
}
