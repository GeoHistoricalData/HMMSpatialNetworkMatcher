...
/**
 * Donne pour une date t, la densité d'arcs nouvellement crées ou détruits (donc crées ou détruits entre t-1 et t)
 * La densité se calcul en utilisant les diagrammes de voronoi
 * @author bcostes
 *
 */
public class TotalPastTransformations {
    ...
    public static  Map<GraphEntity, Double> calculateNodeIndicator(STGraph stgraph, FuzzyTemporalInterval t1, FuzzyTemporalInterval t2,  boolean normalize) {       
        ...
        //récupération des nouveaux arcs et des arcs détruits entre t-1 et t
        ...
        for(STEntity e : stgraph.getEdges()){
            if((e.existsAt(t1) && !e.existsAt(t2)) || (!e.existsAt(t1) && e.existsAt(t2))){
                edgesTransfo.add(e.getGeometry().toGeoxGeometry());
                pop.add(new DefaultFeature(e.getGeometry().toGeoxGeometry()));
            }
        }
        ShapefileWriter.write(pop, "/home/bcostes/Bureau/tmp/dynamics/"+t1.toString()+"_transfo.shp");
        ...
        try {
            ...
            ShapefileWriter.write(pop, "/home/bcostes/Bureau/tmp/dynamics/"+t1.toString()+"_voronoi.shp");
