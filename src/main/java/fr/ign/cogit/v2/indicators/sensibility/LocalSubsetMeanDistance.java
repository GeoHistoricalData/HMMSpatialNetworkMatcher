...
public class LocalSubsetMeanDistance extends ILocalSubsetIndicator{
    public LocalSubsetMeanDistance() {
        this.name = "LocalMeand";
    }
    @Override
    public Map<GraphEntity, Double> calculateSubsetNodeCentrality(
        ...
        if(graph1.getEdgeCount() < graph2.getEdgeCount()){
            return this.calculateSubsetNodeCentrality(graph2, graph1, radius, normalize);
        }
        ...
        Map<GraphEntity, Double> normalizer = new HashMap<GraphEntity, Double>();
        ...
        for(GraphEntity v1: graph2.getVertices()) {
            ...
            for (GraphEntity v2: graph2.getVertices()) {
                if(v1.equals(v2) || v1.getGeometry().toGeoxGeometry().distance(v2.getGeometry().toGeoxGeometry())>radius){
                    continue;
                }
                ...
                if(normalize){
                    normalizer.put(v1, normalizer.get(v1)+1.0);
                }
            }
            ppc.reset(vX1);
        }
        ...
        if(normalize){
            for (GraphEntity w : graph2.getVertices()) {
                result.put(w, result.get(w)  / normalizer.get(w));
            }
        }
        return result;
    }
    ...
    public Map<GraphEntity, Double> calculateSubsetEdgeCentrality(
            JungSnapshot graph1, JungSnapshot graph2, double d, boolean normalize) {
        // TODO Auto-generated method stub
        //on établi la correspondance entre les éléments
        if(graph1.getEdgeCount() < graph2.getEdgeCount()){
            return this.calculateSubsetNodeCentrality(graph2, graph1, d,normalize);
        }
        Map<GraphEntity, Double> result = new HashMap<GraphEntity, Double>();

        Map<GraphEntity, Double> nodesValues = this.calculateSubsetNodeCentrality(graph1, graph2,d, normalize);
    ...
    public static void main(String args[]){
        ...
        ILocalSubsetIndicator ind = new LocalSubsetMeanDistance();

        Map<GraphEntity, Double> indValues = ind.calculateSubsetEdgeCentrality(snap1, snap2, 500, true);
        ...
        ShapefileWriter.write(out, "/home/bcostes/Bureau/subsetmeand_500.shp");
    }
}
