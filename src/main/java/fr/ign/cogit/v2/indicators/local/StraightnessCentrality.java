...
/**
 * dilatation : rapport deuclidenne/dreseau
 * @author bcostes
 * 
 * @param <GraphEntity>
 * @param <GraphEntity>
 */
public class StraightnessCentrality extends ILocalIndicator {

    public StraightnessCentrality() {
        this.name = "Str";
    }
    ...
    public Map<GraphEntity, Double> calculateNodeCentrality(JungSnapshot g, boolean normalize) {
        ...
        for (GraphEntity n1 : g.getVertices()) {
            ...
            for (GraphEntity n2 : g.getVertices()) {
                ...
                //          double pi = g.getNodesWeights().transform(n2);
    ...
    public Map<GraphEntity, Double> calculateNeighborhoodNodeCentrality(
        ...
        for (GraphEntity node : g.getVertices()) {
            ...
            if(normalize){
                values.put(n1, sum / ((double) (g.getKNeighborhood(node, k).size() -1)));
            }
            else{
                values.put(n1, sum );
            }    
