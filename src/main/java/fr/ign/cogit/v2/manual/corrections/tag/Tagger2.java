...
public class Tagger2 {
    ...
    public Tagger2(STGraph stgraph, FuzzyTemporalInterval tend ,
            float max_continuation_value, float min_error_value, Map<FuzzySet, String> shps){
        this.experts = new ArrayList<TaggingSource>();
        this.stgraph = stgraph;
        this.tend = tend;
        this.edges_tagged = new ArrayList<TaggedEdge>();
        /*
         * Création du cadre de Discernement
         */
        this.theta = new ArrayList<TaggingHypothesis>();
        this.theta.add(new TaggingHypothesis(Tag.CONTINUATION));
        ...
        //
        //        this.experts.add(orientation);        this.codec = new DefaultCodec<TaggingHypothesis>(this.theta);
        ...
        if(shps != null){
            ...
            widthFrame.add(this.codec.encode( this.theta.get(1)));
    ...
    private TaggedEdge computeEdgeBelief(Observation to,Observation ho) {
        ...
        this.experts.get(0).affectMassesToHypotheses(to,ho);

        TaggedEdge e= new TaggedEdge();
        if(this.experts.get(0).getMassOf(new byte[] { 1, 0 }) >=0.75){
            e.privilegied_tag = Tag.CONTINUATION;
            e.conflict = 0;
            e.pignistic_value = this.experts.get(0).getMassOf(new byte[] { 1, 0 });
            e.tail = to;
            e.head = ho;
        }

        else{
            e.privilegied_tag = Tag.ERROR;
            e.conflict = 0;
            e.pignistic_value = this.experts.get(0).getMassOf(new byte[] { 0, 1 });
            e.tail = to;
            e.head = ho;
        }
    ...
    public static void main(String args[]){
        ...
        STGraph stgraph = TAGIoManager.deserialize("/home/bcostes/Bureau/tag_copie/tag_corrected_ind.tag");
        ...
        Tagger2 tagger = new Tagger2(stgraph, t2, 10, 20, shps);
        tagger.tag();
        tagger.export("/home/bcostes/Bureau/tag_elargissements.shp");
